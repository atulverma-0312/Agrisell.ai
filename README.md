...
The module is fully state-driven: a 6-step stepper (Produce → Photo → Validation → AI Analysis → Grade → Selling Insight) unlocks step by step; the UI phase (`idle / uploading / validating / validation_failed / ready / analyzing / success / low_confidence / error`) is derived from real upload/validation/service events, never pre-filled. Upload progress, per-check validation status, processing-stage timeline, duplicate-submission lock, farmer-friendly error states (produce not detected, low confidence, network/timeout/API), reset-with-confirmation on crop change, quantity (kg/quintal) validated revenue scenarios, resumable drafts and a `SharedAssessment` hand-off into the simulator are included. Set `VITE_AI_MODE=production` to disable the demo fallback when no real service is configured.
